...
/* As a result, a simple recursive function which asks itself to create a subexpression tree 
whenever it sees an open parenthesis is a feasible solution.   */

AST *build_ast (lexer *lex) {
    /* TODO: Implement me. */
    /* Hint: switch statements are pretty cool, and they work brilliantly with enums. */
    /* get the token */       
    /* use the read_token(lex) */
    read_token(lex);
